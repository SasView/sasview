...
class BasicPage(ScrolledPanel):
    ...
    class ModelTextCtrl(wx.TextCtrl):
        ...
        #while the whole string is selected
    ...
    def onRedo(self, event):
        """
        Restore the previous action canceled 
    ...
    def select_disp_angle(self, event): 
        ...
        if event.GetEventObject() == self.noDisper_rbox:
            if self.noDisper_rbox.GetValue():
                ...
                ## Redraw the model ???
                self._draw_model()
        # Go through the list of dispersion check boxes to 
        #identify which one has changed 
        for p in self.disp_cb_dict:
            ...
            if event.GetEventObject() == self.disp_cb_dict[p]:              
                if self.disp_cb_dict[p].GetValue() == True:
                    ##Temp. FIX for V1.0 regarding changing 
                    ...
                    try:
                        ...
                        return
                    # If any of the two arrays is empty, notify the user 
                    # that we won't proceed 
                    if self.values is None or self.weights is None or \
                         self.values == [] or self.weights == []:
                        msg = "The loaded %s distrubtion is corrupted"
                        msg += " or empty" % p
                        wx.PostEvent(self.parent, StatusEvent(status=msg))
                        return
                    ...
                    # Store the object to make it persist outside the scope 
                    #of this method
                    ...
                    self.model._persistency_dict[p] = [self.values,
                                                       self.weights]
                    self.state.model._persistency_dict[p] = [self.values, 
                                                             self.weights]
                else:
                    self._reset_dispersity()
    ...
    def read_file(self, path):
        ...
        try:
            if path == None:
                msg = " Selected Distribution was not loaded: %s" % path
                wx.PostEvent(self.parent, StatusEvent(status=msg))
                return None, None
            input_f = open(path, 'r')
            buff = input_f.read()
    ...
    def reset_page_helper(self, state):
            ...
            if name == "ArrayDispersion":
                for item in self.disp_cb_dict.keys():
                    if hasattr(self.disp_cb_dict[item], "SetValue") :
                        ...
                        # Set the new model as the dispersion object for 
        ...
        if hasattr(self, "enable_smearer"):
            ## set smearing value whether or not the data contain 
            #the smearing info
    ...
    def _reset_context_menu(self):
        ...
        for name, state in self.state.saved_states.iteritems():
            ...
            ## Add item in the context menu
            id = wx.NewId()
            hint_msg = 'Save model and state %g' % self.number_saved_state
            self.popUpMenu.Append(id, name, hint_msg)
            wx.EVT_MENU(self, id, self.onResetModel)
    ...
    def _copy_parameters_state(self, listtocopy, statelist):
        ...
        :param listtocopy: the list of widgets 's state to copy
        :param statelist: container for the state of widgets that must be copied
    ...
    def _set_dipers_Param(self, event):
        ...
        When the user select yes , this method display a combo box 
        for more selection
    ...
    def _set_sizer_arraydispersion(self):
        ...
        self.sizer4_4.Add(disp1, (iy, ix), (1, 1),  
                          wx.LEFT|wx.EXPAND|wx.ADJUST_MINSIZE, 15)
        # Look for model parameters to which we can apply 
        # an ArrayDispersion model
        # Add a check box for each parameter.
        self.disp_cb_dict = {}
        ix+=1 
        self.noDisper_rbox = wx.RadioButton(self, -1, "None", 
                                            (10, 10), style=wx.RB_GROUP)
        self.Bind(wx.EVT_RADIOBUTTON, self.select_disp_angle, 
                  id=self.noDisper_rbox.GetId())
        #MAC needs SetValue
        self.noDisper_rbox.SetValue(True)
        ...
        for p in self.model.dispersion.keys():
            if not p in self.model.orientation_params:
                ...
                self.state.disp_cb_dict[p] = self.disp_cb_dict[p].GetValue()
                self.Bind(wx.EVT_RADIOBUTTON, self.select_disp_angle, 
                          id=self.disp_cb_dict[p].GetId())
                self.sizer4_4.Add(self.disp_cb_dict[p], (iy, ix), (1,1), 
    ...
    def _lay_out(self):
        ...
        :Note: Mac seems to like this better when self.Layout 
            is called after fitting.
    ...
    def _sleep4sec(self):
        ...
        :Note: This 1sec helps for Mac not to crash on 
            self.layout after self._draw_model

...
class FitArrange:
    ...
    def remove_data(self,data):
        """
            ...
            @param data: Data to remove from the the list of data
        """
...
class Fitting:
    ...
    def fit_engine(self,word):
        ...
        """
        word=word.lower()
        if word =="scipy" or word=="park":
            self.fitType = word
            return True
        else:
            #raise ValueError, "please enter the keyword scipy or park"
            return False
    ...
    def fit(self,pars, qmin=None, qmax=None):
        ...
        if not self.fitType ==None:
            if self.fitType=="scipy":# sans fit
...
class Fitness:
    ...
    def getParam(self):
        return [param() for param in self.pars]
...
def fitHelper(model, pars, x, y, err_y ,qmin=None, qmax=None):
    ...
    f = Fitness(model, pars, x, y, err_y ,qmin, qmax)
    ...
    out, cov_x, info, mesg, success = optimize.leastsq(f,f.getParam(), full_output=1, warning=True)

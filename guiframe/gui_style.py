
"""
Provide the style for guiframe
"""

class GUIFRAME:
    MANAGER_ON = 1
    FLOATING_PANEL = 2
    FIXED_PANEL = 4
    PLOTTIN_ON = 8
    DATALOADER_ON = 16
    TOOL_ON = 32
    SINGLE_APPLICATION = 64
    DEFAULT_STYLE = SINGLE_APPLICATION|DATALOADER_ON|PLOTTIN_ON|FIXED_PANEL
    MULTIPLE_APPLICATIONS = DEFAULT_STYLE|MANAGER_ON

if __name__ == "__main__":
  
    print GUIFRAME.DEFAULT_STYLE
    print GUIFRAME.FLOATING_PANEL
    print GUIFRAME.SINGLE_APPLICATION
    style = GUIFRAME.MULTIPLE_APPLICATIONS
    style &= GUIFRAME.PLOTTIN_ON
    print style == GUIFRAME.PLOTTIN_ON
    style1 = GUIFRAME.MULTIPLE_APPLICATIONS
    style1 &= (~GUIFRAME.MANAGER_ON)
    print style1 == GUIFRAME.DEFAULT_STYLE
    print style1
    
 
  
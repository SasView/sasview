...
def fix_dll(dll: Path) -> None:
    ...
    def match_func(pth: str) -> Optional[str]:
        ...
        if not basename.startswith("cryptography"):
            return None
        elif not basename.startswith("http"):
            return None
        elif not basename.startswith("opt"):
            return None
        elif not basename.startswith("pyopencl"):
            return None
...
def main(args: List[str]) -> int:
    ...
    for app in args:
        ...
        print(f">>> [{name}] Fixing folder names")

...
static PyObject* multifunctionalmodel(PyObject*self, PyObject* args, PyObject* kywds)
{
    py::object return_val;
    int exception_occured = 0;
    PyObject *py_local_dict = NULL;
    static char *kwlist[] = {"x","n","rad_core","sld","thick","slope","fun_type","local_dict", NULL};
    PyObject *py_x, *py_n, *py_rad_core, *py_sld, *py_thick, *py_slope, *py_fun_type;
    int x_used, n_used, rad_core_used, sld_used, thick_used, slope_used, fun_type_used;
    py_x = py_n = py_rad_core = py_sld = py_thick = py_slope = py_fun_type = NULL;
    x_used= n_used= rad_core_used= sld_used= thick_used= slope_used= fun_type_used = 0;
    
    if(!PyArg_ParseTupleAndKeywords(args,kywds,"OOOOOOO|O:multifunctionalmodel",kwlist,&py_x, &py_n, &py_rad_core, &py_sld, &py_thick, &py_slope, &py_fun_type, &py_local_dict))
       return NULL;
    try                              
    {                                
        py_x = py_x;
        double x = convert_to_float(py_x,"x");
        x_used = 1;
        py_n = py_n;
        int n = convert_to_int(py_n,"n");
        n_used = 1;
        py_rad_core = py_rad_core;
        double rad_core = convert_to_float(py_rad_core,"rad_core");
        rad_core_used = 1;
        py_sld = py_sld;
        PyArrayObject* sld_array = convert_to_numpy(py_sld,"sld");
        conversion_numpy_check_type(sld_array,PyArray_DOUBLE,"sld");
        #define SLD1(i) (*((double*)(sld_array->data + (i)*Ssld[0])))
        #define SLD2(i,j) (*((double*)(sld_array->data + (i)*Ssld[0] + (j)*Ssld[1])))
        #define SLD3(i,j,k) (*((double*)(sld_array->data + (i)*Ssld[0] + (j)*Ssld[1] + (k)*Ssld[2])))
        #define SLD4(i,j,k,l) (*((double*)(sld_array->data + (i)*Ssld[0] + (j)*Ssld[1] + (k)*Ssld[2] + (l)*Ssld[3])))
        npy_intp* Nsld = sld_array->dimensions;
        npy_intp* Ssld = sld_array->strides;
        int Dsld = sld_array->nd;
        double* sld = (double*) sld_array->data;
        sld_used = 1;
        py_thick = py_thick;
        PyArrayObject* thick_array = convert_to_numpy(py_thick,"thick");
        conversion_numpy_check_type(thick_array,PyArray_DOUBLE,"thick");
        #define THICK1(i) (*((double*)(thick_array->data + (i)*Sthick[0])))
        #define THICK2(i,j) (*((double*)(thick_array->data + (i)*Sthick[0] + (j)*Sthick[1])))
        #define THICK3(i,j,k) (*((double*)(thick_array->data + (i)*Sthick[0] + (j)*Sthick[1] + (k)*Sthick[2])))
        #define THICK4(i,j,k,l) (*((double*)(thick_array->data + (i)*Sthick[0] + (j)*Sthick[1] + (k)*Sthick[2] + (l)*Sthick[3])))
        npy_intp* Nthick = thick_array->dimensions;
        npy_intp* Sthick = thick_array->strides;
        int Dthick = thick_array->nd;
        double* thick = (double*) thick_array->data;
        thick_used = 1;
        py_slope = py_slope;
        PyArrayObject* slope_array = convert_to_numpy(py_slope,"slope");
        conversion_numpy_check_type(slope_array,PyArray_DOUBLE,"slope");
        #define SLOPE1(i) (*((double*)(slope_array->data + (i)*Sslope[0])))
        #define SLOPE2(i,j) (*((double*)(slope_array->data + (i)*Sslope[0] + (j)*Sslope[1])))
        #define SLOPE3(i,j,k) (*((double*)(slope_array->data + (i)*Sslope[0] + (j)*Sslope[1] + (k)*Sslope[2])))
        #define SLOPE4(i,j,k,l) (*((double*)(slope_array->data + (i)*Sslope[0] + (j)*Sslope[1] + (k)*Sslope[2] + (l)*Sslope[3])))
        npy_intp* Nslope = slope_array->dimensions;
        npy_intp* Sslope = slope_array->strides;
        int Dslope = slope_array->nd;
        double* slope = (double*) slope_array->data;
        slope_used = 1;
        py_fun_type = py_fun_type;
        PyArrayObject* fun_type_array = convert_to_numpy(py_fun_type,"fun_type");
        conversion_numpy_check_type(fun_type_array,PyArray_LONG,"fun_type");
        #define FUN_TYPE1(i) (*((long*)(fun_type_array->data + (i)*Sfun_type[0])))
        #define FUN_TYPE2(i,j) (*((long*)(fun_type_array->data + (i)*Sfun_type[0] + (j)*Sfun_type[1])))
        #define FUN_TYPE3(i,j,k) (*((long*)(fun_type_array->data + (i)*Sfun_type[0] + (j)*Sfun_type[1] + (k)*Sfun_type[2])))
        #define FUN_TYPE4(i,j,k,l) (*((long*)(fun_type_array->data + (i)*Sfun_type[0] + (j)*Sfun_type[1] + (k)*Sfun_type[2] + (l)*Sfun_type[3])))
        npy_intp* Nfun_type = fun_type_array->dimensions;
        npy_intp* Sfun_type = fun_type_array->strides;
        int Dfun_type = fun_type_array->nd;
        long* fun_type = (long*) fun_type_array->data;
        fun_type_used = 1;
        /*<function call here>*/     
        
                            double bes,fun,alpha,beta,f,vol,qr,r,contr,f2;
                            double pi;
                            int i,j;
                                    ...
                                    case 'exp' :
                                                ...
                                                for (j=0; j<2; j++){
                                                    ...
                                                    if(qr == 0.0){
                                                        ...
                                                        if (fabs(slope[i]) > 0.0 ){
                                                            fun = 3.0 * ((alpha*alpha - qr * qr) * sin(beta) - 2.0 * alpha * qr * cos(qr))/ ((alpha * alpha + qr * qr) * (alpha * alpha + qr * qr) * qr);
                                                            bes = -3.0 * (alpha * sin(qr) - qr * cos(qr)) / ((alpha * alpha + qr * qr) * qr);
                                                            }
                                                        else {
                                    ...
                                    default :
                                                ...
                                                for (j=0; j<2; j++){
                                                    ...
                                                    if(qr == 0.0){
                                                        ...
                                                        bes = - 3.0 * (sin(qr) - qr * cos(qr)) / (qr * qr * qr);
                                                        if (fabs(slope[i]) > 0.0 ){
                                                            fun = -3.0 * r * (2.0*qr*sin(qr)-((qr*qr)-2.0)*cos(qr))/(qr * qr * qr * qr);
                                                            }
                                                        }
                                                    if (j == 1){
                                                        bes = - bes;
...
{
    {"multifunctionalmodel",(PyCFunction)multifunctionalmodel , METH_VARARGS|METH_KEYWORDS},
    {NULL,      NULL}        /* Sentinel */
};

PyMODINIT_FUNC initmodel_func_ext(void)
{
    
    Py_Initialize();
    import_array();
    PyImport_ImportModule("numpy");
    (void) Py_InitModule("model_func_ext", compiled_methods);
}

...
static PyObject * run(CLamellarModel *self, PyObject *args) {
    ...
    self->model_pars.sld_sol = PyFloat_AsDouble( PyDict_GetItemString(self->params, "sld_sol") );
    self->model_pars.scale = PyFloat_AsDouble( PyDict_GetItemString(self->params, "scale") );
    self->model_pars.bi_thick = PyFloat_AsDouble( PyDict_GetItemString(self->params, "bi_thick") );
    self->model_pars.background = PyFloat_AsDouble( PyDict_GetItemString(self->params, "background") );
    self->model_pars.sld_bi = PyFloat_AsDouble( PyDict_GetItemString(self->params, "sld_bi") );
	...
	if( PyList_Check(pars)==1) {
		...
		return Py_BuildValue("d",lamellar_analytical_2D(&(self->model_pars),q_value,phi_value));
		...
		return Py_BuildValue("d",lamellar_analytical_1D(&(self->model_pars),q_value));
	}	
}
...
static PyObject * runXY(CLamellarModel *self, PyObject *args) {
	...
	if( PyList_Check(pars)==1) {
		...
		return Py_BuildValue("d",lamellar_analytical_2DXY(&(self->model_pars),qx_value,qy_value));
		...
		return Py_BuildValue("d",lamellar_analytical_1D(&(self->model_pars),qx_value));
	}	
}

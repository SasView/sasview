...
typedef struct {
    ...
    PyObject * log;
    /// Model parameters
	MultiShellParameters model_pars;
} CMultiShellModel;
...
static PyObject * run(CMultiShellModel *self, PyObject *args) {
    ...
    self->model_pars.n_pairs = PyFloat_AsDouble( PyDict_GetItemString(self->params, "n_pairs") );
    self->model_pars.w_thickness = PyFloat_AsDouble( PyDict_GetItemString(self->params, "w_thickness") );
    self->model_pars.s_thickness = PyFloat_AsDouble( PyDict_GetItemString(self->params, "s_thickness") );
	...
	if( PyList_Check(pars)==1) {
		...
		return Py_BuildValue("d",multishell_analytical_2D(&(self->model_pars),q_value,phi_value));
		...
		return Py_BuildValue("d",multishell_analytical_1D(&(self->model_pars),q_value));
	}	
}
...
static PyObject * runXY(CMultiShellModel *self, PyObject *args) {
	...
	if( PyList_Check(pars)==1) {
		...
		return Py_BuildValue("d",multishell_analytical_2DXY(&(self->model_pars),qx_value,qy_value));
		...
		return Py_BuildValue("d",multishell_analytical_1D(&(self->model_pars),qx_value));
	}	
}

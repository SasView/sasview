...
typedef struct {
    ...
    PyObject * log;
    /// Model parameters
	FlexibleCylinderParameters model_pars;
} CFlexibleCylinderModel;
...
static PyObject * run(CFlexibleCylinderModel *self, PyObject *args) {
    ...
    self->model_pars.kuhn_length = PyFloat_AsDouble( PyDict_GetItemString(self->params, "kuhn_length") );
	...
	if( PyList_Check(pars)==1) {
		...
		return Py_BuildValue("d",flexible_cylinder_analytical_2D(&(self->model_pars),q_value,phi_value));
		...
		return Py_BuildValue("d",flexible_cylinder_analytical_1D(&(self->model_pars),q_value));
	}	
}
...
static PyObject * runXY(CFlexibleCylinderModel *self, PyObject *args) {
	...
	if( PyList_Check(pars)==1) {
		...
		return Py_BuildValue("d",flexible_cylinder_analytical_2DXY(&(self->model_pars),qx_value,qy_value));
		...
		return Py_BuildValue("d",flexible_cylinder_analytical_1D(&(self->model_pars),qx_value));
	}	
}

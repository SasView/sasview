...
static PyObject * run(CStickyHSStructure *self, PyObject *args) {
    ...
    self->model_pars.stickiness = PyFloat_AsDouble( PyDict_GetItemString(self->params, "stickiness") );
    self->model_pars.volfraction = PyFloat_AsDouble( PyDict_GetItemString(self->params, "volfraction") );
    self->model_pars.perturb = PyFloat_AsDouble( PyDict_GetItemString(self->params, "perturb") );
	...
	if( PyList_Check(pars)==1) {
		...
		return Py_BuildValue("d",StickyHS_analytical_2D(&(self->model_pars),q_value,phi_value));
		...
		return Py_BuildValue("d",StickyHS_analytical_1D(&(self->model_pars),q_value));
	}	
}
...
static PyObject * runXY(CStickyHSStructure *self, PyObject *args) {
	...
	if( PyList_Check(pars)==1) {
		...
		return Py_BuildValue("d",StickyHS_analytical_2DXY(&(self->model_pars),qx_value,qy_value));
		...
		return Py_BuildValue("d",StickyHS_analytical_1D(&(self->model_pars),qx_value));
	}	
}

...
ProlateModel :: ProlateModel() {
	scale      = Parameter(1.0);
	major_core     = Parameter(100.0, true);
	major_core.set_min(0.0);
	minor_core     = Parameter(50.0, true);
	minor_core.set_min(0.0);
	major_shell   = Parameter(110.0, true);
	major_shell.set_min(0.0);
	minor_shell    = Parameter(60.0, true);
 ...
 */
double ProlateModel :: operator()(double q) {
	double dp[8];
					...
					for(int m=0; m< (int)weights_theta.size(); m++) {
						...
						for(int n=0; n< (int)weights_phi.size(); n++) {
							...
							double _ptvalue = weights_major_core[i].weight *weights_minor_core[j].weight
								...
								* prolate_analytical_2DXY(&dp, qx, qy);

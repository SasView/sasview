...
COblateModel_init(COblateModel *self, PyObject *args, PyObject *kwds)
{
    if (self != NULL) {
        ...
        self->model = new OblateModel();
        
        // Initialize parameter dictionary
        PyDict_SetItemString(self->params,"major_core",Py_BuildValue("d",200.00000000));
        PyDict_SetItemString(self->params,"scale",Py_BuildValue("d",1.00000000));
        PyDict_SetItemString(self->params,"minor_core",Py_BuildValue("d",20.00000000));
        PyDict_SetItemString(self->params,"axis_theta",Py_BuildValue("d",1.00000000));
        ...
        PyDict_SetItemString(self->params,"major_shell",Py_BuildValue("d",250.00000000));
        PyDict_SetItemString(self->params,"contrast",Py_BuildValue("d",0.00000100));
        PyDict_SetItemString(self->params,"minor_shell",Py_BuildValue("d",30.00000000));
...
static PyMemberDef COblateModel_members[] = {
    {"params", T_OBJECT, offsetof(COblateModel, params), 0,
     "Parameters"},
	{"dispersion", T_OBJECT, offsetof(COblateModel, dispersion), 0,
	  "Dispersion parameters"},     
...
static PyObject *evaluateOneDim(OblateModel* model, PyArrayObject *q){
 ...
 * @return: numpy array object 
 */
 static PyObject * evaluateTwoDimXY( OblateModel* model, 
	...
	if (PyArray_Check(x) && PyArray_Check(y)) {
        ...
        return PyArray_Return(result); 
		    ...
		    PyErr_SetString(COblateModelError, 
                   "COblateModel.evaluateTwoDimXY couldn't run.");
	        return NULL;
		}      	
}
/**
 ...
 *
 */ 
static PyObject * evalDistribution(COblateModel *self, PyObject *args){
	PyObject *qx, *qy;
	PyArrayObject * pars;
	int npars ,mpars;
	...
	if ( !PyArg_ParseTuple(args,"O",&pars) ) {
	    PyErr_SetString(COblateModelError, 
	    	"COblateModel.evalDistribution expects a q value.");
		return NULL;
	}
    // Check params
    ...
    if(PyArray_Check(pars)==1) {
	    ...
	    if(npars==1) {
		    ...
		    PyErr_SetString(COblateModelError, 
                   "COblateModel.evalDistribution expect numpy array of one dimension.");
	        return NULL;
		}
	    ...
	    if(mpars!=2) {
	    	PyErr_SetString(COblateModelError, 
	    		"COblateModel.evalDistribution expects a list of dimension 2.");
	    	return NULL;
	    }
	     ...
	     if (PyArray_Check(qx) && PyArray_Check(qy)) {
		    ...
		    PyErr_SetString(COblateModelError, 
                   "COblateModel.evalDistribution expect 2 numpy arrays in list.");
	        return NULL;
	     }
	}
	PyErr_SetString(COblateModelError, 
 ...
 */
static PyObject * calculate_ER(COblateModel *self) {
...
static PyObject * set_dispersion(COblateModel *self, PyObject *args) {
	PyObject * disp;
	const char * par_name;

	if ( !PyArg_ParseTuple(args,"sO", &par_name, &disp) ) {
	    PyErr_SetString(COblateModelError,
	    	"COblateModel.set_dispersion expects a DispersionModel object.");
		return NULL;
	}

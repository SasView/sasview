...
class DialogAbout(wx.Dialog):
    ...
    def __init__(self, *args, **kwds):
        ...
        if os.name == 'nt':
            self.bitmap_logo = wx.StaticBitmap(self, -1, wx.Bitmap(os.path.join("images",config._corner_image)))
        else:
            self.bitmap_logo = wx.StaticBitmap(self, -1, wx.Bitmap(os.path.join("images",config._corner_image)))
...
#       self.text_ctrl_acknowledgement.SetValue(__acknowledgement__)
        ...
        logo = wx.Bitmap(os.path.join("images",config._nsf_logo))
        self.bitmap_button_nsf.SetBitmapLabel(logo)
        logo = wx.Bitmap(os.path.join("images",config._danse_logo))
        self.bitmap_button_danse.SetBitmapLabel(logo)
        logo = wx.Bitmap(os.path.join("images",config._inst_logo))

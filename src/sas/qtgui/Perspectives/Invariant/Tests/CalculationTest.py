...
class TestInvariantCalculation:
    ...
    def test_no_extrapolation_early_return(
        self, extrapolate_checkbox, extrapolate_param, compute_func, expected
    ):
    ...
    def test_compute_low(self, mocker, low_guinier, low_fit, low_fix, expected_function):
        ...
        mock_calculator.set_extrapolation.assert_called_once_with(
            range = "low",
            npts = self.window._low_points,
            function = expected_function,
            power = power if low_fix else None
        )
    ...
    def test_compute_high(self, mocker, high_fit, high_fix):
        ...
        mock_calculator.set_extrapolation.assert_called_once_with(
            range = "high",
            npts = self.window._high_points,
            function = "power_law",
            power = power if high_fix else None
        )
        assert (qstar, qstar_err, success) == (1.0, 0.1, True)

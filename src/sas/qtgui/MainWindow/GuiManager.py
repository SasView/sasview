...
class GuiManager:
    ...
    def _sasbdb_guinier_source_data_1d(self):
        ...
        if hasattr(self, '_data_manager'):
            try:
                ...
                logger.debug("Could not resolve SASBDB Guinier source data",
                             exc_info=True)
        return None
    ...
    def actionSASBDB(self):
        ...
        # Prefer perspective auto-fill when available (Fitting PR).
        if self._current_perspective is not None:
            get_sasbdb = getattr(
                self._current_perspective, 'getSASBDBData', None)
            if callable(get_sasbdb):
                try:
                    perspective_data = get_sasbdb()
                    if perspective_data is not None:
                        export_data = perspective_data
                except Exception as e:
                    logger.warning(
                        f"Could not collect data from perspective: {e}")
        ...
        if not export_data.samples and hasattr(self, '_data_manager'):
            try:
                ...
                if stored_data:
                    ...
                    if data_items:
                        ...
                        if isinstance(data, (Data1D, Data2D)):
                            sample, instrument = collector.collect_from_data(
                                data)
                ...
                logger.warning(
                    f"Could not collect data from data manager: {e}")

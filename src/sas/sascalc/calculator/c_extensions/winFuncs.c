/* winFuncs.c
   Adding functions missing from windows math library
...
double fmax(double x, double y){
	//Doesn't exactly match BSD as if one or other value is NaN behaviour is undefined
	//BSD returns the other value if one is NaN and NaN if both are NaN
	//probably wouldn't want to rely on that in any case.
	if (x > y) {
		//x is greater than y
		return x;
	} else if (y > x) {
...
double trunc(double x){
	...
	if (x > 0){
		...
		//negative - need ceiling
		return ceil(x);
	} else {
		//x is zero or infinity, return x
		return x;
	}

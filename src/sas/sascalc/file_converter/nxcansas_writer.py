...
class NXcanSASWriter(Reader):
    ...
    def write(self, dataset, filename):
        ...
        writer_note = 'Data file generated by SasView v{0}'.format(VERSION)
        notes = [np.string_(writer_note)]
        if np.isscalar(data_info.notes) and data_info.notes:
            # Handle scalars that aren't empty arrays, None, '', etc.
            notes.append(np.string_(data_info.notes[0]))
        elif data_info.notes:
            # Handle iterables that aren't empty
            notes.extend([np.string_(n) for n in data_info.notes])

...
def add_gaussian_noise(x, dx, seed=None):
    ...
    :param x: array-like, input intensity values
    :param dx: array-like, sigma of Guassian uncertainties associated with the intensities
    :param seed: int or None, optional, random seed for reproducibility (default: None)
    :return: array-like, data with added Gaussian noise
    """
...
class sizeDistribution:
    def __init__(self, data: Data1D):
        ...
        self.MaxEnt_statistics = {"volume": 0.0, "volume_err": 0.0, "mean": 0.0, "median": 0.0, "mode": 0.0}
    ...
    def model(self, value: str):
        ...
        if value != "ellipsoid":
            logger.info(
                "Model is hard coded to ellipsoid for the time being. Please only use ellipsoid. Setting model to ellipsoid."
            )
            self._model = "ellipsoid"
        else:
            self._model = value
    ...
    def generate_model_matrix(self, moddata: Data1D):
        ...
        :param moddata: Data1D object that has the data trimmed depending on background subtraction or powerlaw subtracted from the data. Also self.qMin and self.qMax.
        """
    ...
    def prep_maxEnt(self, sub_intensities: Data1D, full_fit: bool = False, nreps: int = 10, rngseed=None):
        ...
        :param full_fit: bool,
            - if True, add Gaussian noise to the subtracted intensities and run maxEnt for nreps iterations to get a distribution of bin magnitudes
    ...
    def run_maxEnt(self, maxentdata: Data1D, intensities: list, BinsBack: np.array, sigma: np.array):
        ...
        for nint, intensity in enumerate(intensities):
            ...
            try:
                ...
                if not converged:
                    logger.warning(
                        "Maximum Entropy did not converge. Try increasing the weight factor to increase the weighting effect."
                    )
            except ZeroDivisionError:
                logger.error(
                    "Divide by Zero Error occured in maximum entropy fitting. Try increasing the weight factor to increase the error weighting"
                )
        ...
        if len(intensities) == 1:
            ...
            logger.error(
                "The length of the intensity array is 0. Did you run prep_maxEnt before run_maxEnt? Check that intensities is an array of at least length 1."
            )

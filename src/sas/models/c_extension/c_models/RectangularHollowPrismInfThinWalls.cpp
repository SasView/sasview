...
typedef struct {
    ...
    double sldSolv;
    double background;
} RectangularHollowPrismInfThinWallsParameters;


RectangularHollowPrismInfThinWallsModel :: RectangularHollowPrismInfThinWallsModel() {
    scale      = Parameter(1.0);
 ...
 */
double RectangularHollowPrismInfThinWallsModel :: operator()(double q) {
    ...
    for (int i=0; i < (int)weights_short_side.size(); i++) {
        ...
        for (int j=0; j < (int)weights_b2a_ratio.size(); j++) {
            ...
            for (int k=0; k < (int)weights_c2a_ratio.size(); k++) {
        	    ...
        	    // Un-normalize  by volume = 2*a*b + 2*a*c + 2*b*c

	            double a = dp[1];
	            double b = dp[1] * dp[2];
	            double c = dp[1] * dp[3];
                double vol_i = (2.0*a*b) + (2.0*a*c) + (2.0*b*c);

        	    sum += weights_short_side[i].weight *
        	           weights_b2a_ratio[j].weight *
        	           weights_c2a_ratio[k].weight *
        	           RectangularHollowPrismInfThinWalls(dp, q) *
                       vol_i;
 ...
 * @return: function value
 */
double RectangularHollowPrismInfThinWallsModel :: operator()(double qx, double qy) {
    return 1.0;
}
 ...
 * @return: function value
 */
double RectangularHollowPrismInfThinWallsModel :: evaluate_rphi(double q, double phi) {
    double qx = q*cos(phi);
    double qy = q*sin(phi);
 ...
 */
double RectangularHollowPrismInfThinWallsModel :: calculate_ER() {
    return 1.0;

}
double RectangularHollowPrismInfThinWallsModel :: calculate_VR() {
  return 1.0;
}

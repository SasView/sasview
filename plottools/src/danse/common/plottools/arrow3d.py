...
class Arrow3D(FancyArrowPatch):
    ...
    def __init__(self, base, xs, ys, zs, colors, *args, **kwargs):
        ...
        if base != None:
            ...
            base.canvas.mpl_connect('button_press_event', self.onLeftDown)
            base.canvas.mpl_connect('button_release_event', self.onLeftUp)
    ...
    def draw(self, renderer, rasterized=True):
        ...
        for i in xrange(len(xs3d)):
            xs, ys, _ = proj3d.proj_transform(xs3d[i], ys3d[i], zs3d[i], 
                                               renderer.M)

...
from sas import config as config

#REG_PROGRAM = """{app}\MYPROG.EXE"" ""%1"""
APPLICATION = str(config.__appname__) + '.exe'
AppName = str(config.__appname__)
AppVerName = str(config.__appname__) + '-' + str(config.__version__)
Dev = ''
if AppVerName.lower().count('dev') > 0:
    Dev = '-Dev'
AppPublisher = config._copyright
AppPublisherURL = config._homepage
AppSupportURL = config._homepage
AppUpdatesURL = config._homepage
ChangesEnvironment = 'true'
DefaultDirName = os.path.join("{pf}" , AppName+Dev)
DefaultGroupName = os.path.join(config.DefaultGroupName, AppVerName)

OutputBaseFilename = config.OutputBaseFilename
SetupIconFile = config.SetupIconFile_win
...
def write_file():
    ...
    msg += """Source: "dist\sasview\plugin_models\*";\tDestDir: "{%%USERPROFILE}\.sasview\plugin_models";\t"""
    msg += """Flags: recursesubdirs createallsubdirs\n"""
    msg += """Source: "dist\sasview\custom_config.py";\tDestDir: "{%%USERPROFILE}\.sasview\config";\t"""
    msg += """Flags: recursesubdirs createallsubdirs\n"""
...
def generate_installer():
    ...
    TEMPLATE += "ArchitecturesInstallIn64BitMode=x64\n"
